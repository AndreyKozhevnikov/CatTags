...
namespace DXCatBase.Module.Controllers {
    public class GeneratePromptsController :ViewController {
        ...
        private void GenerateOneFeaturesPrompt_Execute(object sender, SimpleActionExecuteEventArgs e) {
            var feature = View.CurrentObject as Feature;
            if(feature == null) {
                return;
            }
            var os = Application.CreateObjectSpace<Feature>();
        ...
        void GeneratePromtForFeature(Feature feature, IObjectSpace os, ChatClient client) {
            var preMessage = "You will receive several texts with subjects in JSON format. Most of them will fall under a single category. " +
                "Your task is to create a prompt to evaluate whether future texts also belong to this category. Ensure that your prompt includes keywords to help identify texts within this category. " +
                "Note that some texts may appear in this category by mistake, so if a text differs significantly from the others, skip it." +
                "Put only prompt in your response, don't need explanations" +
                "The prompt should contain 5 sentences that descibe the most of the texts and 10-15 keywords";
            var realFeature = os.GetObject(feature);
            var data = GetTicketData(realFeature, os);
            if(data == null) {
                return;
            }
            var result = client.CompleteChat(preMessage, data);
            var promptText = result.Value.Content[0].Text;
            var featurePrompt = os.CreateObject<FeaturePrompt>();
            featurePrompt.Prompt = promptText;
            featurePrompt.Feature = realFeature;
